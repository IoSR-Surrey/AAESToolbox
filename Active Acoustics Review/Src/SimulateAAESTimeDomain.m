...
%% High-level params
rir_read_dir = "Active Acoustics Review/Generated AAES RIRs/Room Condition 2/";
output_dir = "Active Acoustics Review/AAES Receiver RIRs/";
output_filename = "AAES_Inline_TI_-2dB.wav";
is_time_varying = false;
config.gain = -2; % dB
config.gainIsRelativeToGBI = true;
rt_ratio = 1.0;
impulse_length_sec = 4;
bit_depth = 32;
...
% Unit impulse:
anechoic_signal = zeros(impulse_length_sec * config.fs, 1);
anechoic_signal(1) = 1;
fs_an = config.fs;
...
% Time Variation
if (is_time_varying)
    FDN.modulationFrequency = 1.0;  % Hz
    FDN.modulationAmplitude = 1.0;
    FDN.spread = 20.0;
else
